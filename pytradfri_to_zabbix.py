...
def zabbix_tradfri_discovery_send( devid, devtype, dev_name, created_at,  zahost, monhost):
    ...
    pprint.pp(zapacket)
    send_zabbix_packet(zapacket , zahost)
...
def main():
    ...
    print("*** START dev.raw")
    key_prefix = 'tradfri'
    device_item_list = ['id','name','created_at', 'ota_update_state',
    ...
    for dev in devices:
        key_begin = f'{key_prefix}.device.{app_type_dict[dev.application_type]}' \
            + f'.[{dev.id}_{int (datetime.timestamp(dev.created_at))},'
        #
        if device_discovery:
            ...
            for k in dev.raw: #device_item_list:
                ...
                if k[0] in device_item_list and k[1] is not None:
                    ivlist += [ZabbixMetric( config['monitored_hostname'],
                                f'{key_begin},{k[0]}', k[1], epoch_timestamp)]
            ...
            for k in dev.device_info.raw:
                ...
                if k[0] in device_info_item_list and k[1] is not None:
                    ivlist += [ZabbixMetric( config['monitored_hostname'],
                            f'{key_begin},device_info,{k[0]}',
                            k[1],epoch_timestamp)]
            if dev.light_control is not None:
                print ('raw', dev.light_control.raw , type(dev.light_control.raw))
                print ('len', len(dev.light_control.raw))
                for k in dev.light_control.raw[0]:
                    if list_all_items:
                        print(k[0], k[1],type(k),)
                    if k[0] in device_light_item_list and k[1] is not None:
                        ivlist += [ZabbixMetric( config['monitored_hostname'],
                                f'{key_begin},light_control,{k[0]}',
                                k[1],epoch_timestamp)]            
            if config['print_zabbix_send']:
                pprint.pp(ivlist)
